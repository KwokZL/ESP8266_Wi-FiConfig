...
void setup() {
  ...
  WiFi.hostname("Wi-Fi_Device");
  WiFi.begin(ssid,psw);
  Serial.println(ssid);
  Serial.println("i");
  pinMode(13, OUTPUT);//指示灯，你可以自行定义
...
Serial.println(psw);
  ...
  if(i>=10){//10秒内未连接到Wi-Fi则开启联网页面
  WiFi.mode(WIFI_AP); 
  WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
  WiFi.softAP("Device Connect", "");
  //WiFi.mode(WIFI_STA);
...
if(WiFi.status() == WL_CONNECTED){//运行状态
  //从这里可以配置在连接Wi-Fi后需要运行加载的组件（相当于setup）
  server.begin();
...
if(WiFi.status() != WL_CONNECTED){
  //处于待连接Wi-Fi的状态。这里是指示灯慢速闪烁，表示等待配置Wi-Fi
  digitalWrite(13, LOW);
  delay(1000);
  digitalWrite(13, HIGH);
  delay(1000);
    ...
    <div>\
        <div style='line-height:90px' id='mainDiv'>\
            ...
            <form action='/connectWifi' method='get'>\
                <div style='line-height:35px'><span><span class='red'>*</span>选择您的Wi-Fi：</span><select name='ssid' required='' id='sleSsid'>\
